/* =============================================================
   ...
   fetch('/api/tasks') replaced with getTasks() from db.js
   ============================================================= */
...
function render() {
  ...
  if (hdr) hdr.innerHTML = WDAYS.map((d) => `<div class="cal-day-name">${d}</div>`).join("");
  ...
  for (let d = 1; d <= days; d++) {
    ...
    const dots =
      ...
      (tasks.length > 3 ? `<div class="cal-dot">+${tasks.length - 3}</div>` : "");
...
function selectDay(cell) {
  ...
  if (titleEl) {
    titleEl.textContent = parseLocal(iso).toLocaleDateString("en-US", {
      weekday: "long", month: "long", day: "numeric",
    });
  }

  const listEl = panel.querySelector(".day-task-list");
  if (listEl) {
    if (!tasks.length) {
      listEl.innerHTML = `<p style="color:var(--slate);font-size:0.85rem">No tasks scheduled.</p>`;
    } else {
